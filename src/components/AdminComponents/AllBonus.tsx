...
const AllBonus = ({bonuses}: any) => {
    
    const pendingBonus = bonuses.filter((deposit: { status: string; }) => deposit.status === 'pending')
    const processedBonus = bonuses.filter((deposit: { status: string; }) => deposit.status !== 'pending')
    ...
    <>
        <main className="h-screen text-xs md:text-sm xl:text-base">
            <div className="h-1/2 overflow-y-auto px-2 sm:px-4 md:px-6 xl:px-8 py-4 special">
                <p className="text-base md:text-lg xl:text-xl font-semibold text-[#020100]">Pending Bonuses</p>
                <div className="flex flex-col gap-y-3 mt-4">
                {pendingBonus && pendingBonus.map((pending: any) => (
                       ...
                       <div className="flex items-center justify-between cursor-pointer">
                           <Link href={`history/${pending.id}`} className="flex gap-x-1 items-center">
                               ...
                               <div className="flex flex-col gap-y-0.5">
                                   <p className="text-[#141619] text-xs md:text-sm xl:text-base font-medium capitalize">{pending.coin === "bitcoin" ? "Bitcoin" : pending.coin === "ethereum" ? "Ethereum" : pending.coin === "binance" ? "Binancecoin" : pending.coin === "tron" ? "Tron" : pending.coin.includes('usd') ? "Tether" : pending.coin === "ada" ? "Cardano" : pending.coin === "solana" ? "Solana" : pending.coin === "lite" ? "Litecoin" : pending.coin === "doge" ? "Doge" : "Coin"} was given</p>
            ...
            <div className="h-1/2 overflow-y-auto px-4 md:px-6 xl:px-8 py-4 special">
                <p className="text-base md:text-lg xl:text-xl font-semibold text-[#020100]">Processed Bonuses</p>
                <div className="flex flex-col gap-y-3 mt-4">
                    {processedBonus && processedBonus.map((processed: any) => ( 
