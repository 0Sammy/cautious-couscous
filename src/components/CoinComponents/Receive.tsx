    ...
    <main className="fixed bg-black bg-opacity-80 flex items-center justify-center z-[100] inset-0">
      <div className="relative w-[90%] sm:w-[70%] md:w-[50%] lg:w-[40%] 2xl:w-[30%] bg-white p-4 md:p-8 rounded-lg">
        <div className="flex items-center justify-between p-2 pb-4 border-b border-slate-500">
          <p className="uppercase font-semibold">RECEIVE {coin === "bitcoin" ? "BTC" : coin === "ethereum" ? "ETH" : coin === "binance" ? "BNB" : coin === "tron" ? "TRX" : coin === "usdtt" ? "usdt (trc20)" : coin === "usdte" ? "usdt (erc20)" : coin === "ada" ? "ada" : coin === "solana" ? "SOL" : coin === "lite" ? "LTC" : coin === "doge" ? "DOGE" : "coin"}</p>
        ...
        <div className="mt-4 flex justify-center">
          <button className={`${isCopied ? "bg-green-600" : "bg-primary"} text-white rounded-lg px-6 md:px-8 xl:px-10 border border-primary hover:bg-inherit hover:text-primary duration-300 py-3`} onClick={handleCopyClick}>
            Copy
          </button>
        </div>
        ...
        <div className="flex justify-end mt-10">
          <button className="bg-red-600 border border-red-600 hover:bg-inherit hover:text-red-600 duration-300 text-white rounded-lg px-6 md:px-8 xl:px-10 py-3" onClick={toggleFunction}>
            Close
          </button>
        </div>
