import Image from "next/image";

//Import Needed Images
import shield from "../../../public/Images/shield.svg";


const OurVision = () => {
    return ( 
        <main className="mt-24 bg-[#242426] rounded-[20px] md:rounded-[30px] py-10 md:py-20 px-4 sm:px-6 md:px-8 xl:px-10 text-white flex flex-col gap-y-5 items-center md:items-start md:gap-y-0 md:flex-row md:justify-between md:gap-x-5">
            <h3 className="font-semibold text-xl md:text-2xl xl:text-3xl w-full md:w-[25%]">Our vision</h3>
            <div className="flex flex-col items-center md:flex-row">
                <div className="flex flex-col gap-y-5">
                  <p className="text-3xl md:text-4xl lg:text-5xl xl:text-[3.5rem] font-bold">The standard for trust in Web3</p>  
                  <p className="text-sm md:text-base xl:text-lg font-medium">Our vision is to become the most trusted hub for Web3 experiences.</p>
                </div>
                <Image src={shield} alt="Image" className="size-52" />
            </div>
        </main>
     );
}
 
export default OurVision;