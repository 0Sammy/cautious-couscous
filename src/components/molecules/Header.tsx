...
const Header = ({userDetails}: any) => {
    ...
    return ( 
        <main >
            ...
            {userOpen && <> <button onClick={toggleUser} type="button" className="top-0 left-0 bg-black/10 w-full h-screen fixed z-[998]"/><div className="z-[999] absolute bg-white border border-slate-200 rounded-md p-4 right-7 top-[3.5rem] text-xs md:text-sm xl:text-base">
            ...
            {settingsOpen && <><button onClick={toggleSettings} type="button" className="top-0 left-0 bg-black/10 w-full h-screen fixed z-[998]"/><div className="z-[999] absolute bg-white border border-slate-200 rounded-md p-4 right-[5rem] top-[3.5rem] flex flex-col gap-y-3">
